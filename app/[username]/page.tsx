...
export default function PublicProfilePage() {
    ...
    const links = useQuery(api.links.listByUsername, { username })

    if (user === undefined || links === undefined) {
        return (
            <div className="min-h-screen bg-black flex items-center justify-center">
                <div className="animate-pulse text-zinc-500 font-outfit uppercase tracking-widest text-sm">
                    Loading profile...
                </div>
            </div>
        )
    ...
    if (user === null) {
        return (
            <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6 text-center space-y-6">
                <h1 className="text-4xl font-black text-white font-outfit tracking-tighter">404</h1>
                <p className="text-zinc-500 font-medium">This profile doesn't exist.</p>
                <Button asChild className="rounded-full bg-white text-black hover:bg-zinc-200">
                    <a href="/">Create your own Linkroot</a>
                </Button>
            </div>
        )
    }

    const pattern = PATTERNS.find((p) => p.id === user.theme) || PATTERNS[0]
    const isLight = pattern.isLight

    return (
        <div className="min-h-screen bg-black text-white font-outfit selection:bg-white selection:text-black flex flex-col items-center justify-center p-6 dark relative overflow-hidden">
        <Background themeId={user.theme} />
            <div className="max-w-xl w-full pt-10 pb-12 relative z-10">
                <div className="flex justify-center">
                    <SocialCard
                        image={(user.image as string) || "https://github.com/shadcn.png"}
                        title={`${user.name || user.username}'s Linkroot`}
                        name={(user.name || user.username) as string}
                        pitch={(user.bio || "Welcome to my Linkroot profile.") as string}
                        buttons={links.map((link: any) => ({
                            label: link.title,
                        ...
                        >
                            <Badge
                                ...
                                className={cn(
                                    ...
                                    isLight 
                                        ? "border-zinc-200 bg-white/50 text-zinc-900 group-hover:border-zinc-400 group-hover:text-zinc-700" 
                                        : "border-zinc-800 bg-zinc-950/50 text-zinc-500 group-hover:border-zinc-600 group-hover:text-zinc-300"
                                )}
                            >
                                <svg
                                    width="12"
            ...
            {user.chatbotEnabled && <PublicChat username={user.username!} />}
        </div>
    )
}
