...
const LinkrootLogo = ({ className }: { className?: string }) => (
	<div className={cn("flex items-center gap-2", className)}>
		...
		>
			...
			<defs>
				<clipPath id="clip0-footer">
					<rect width="200" height="200" fill="white" />
				</clipPath>
			</defs>
		</svg>
		<span className="text-base font-semibold tracking-tight font-outfit">Linkroot</span>
	</div>
);
...
const navLinks = [
	...
	{ href: "/blog/privacy", label: "Privacy" },
];
...
export function Footer() {
	return (
		<footer className="relative z-10 mx-auto max-w-5xl border-x border-border/15">
			<div className="relative z-10 flex flex-col gap-6 py-6 px-4 md:px-6">
				<div className="flex items-center justify-between">
					<a 
						href="/" 
						className="relative z-10 flex items-center gap-2 hover:opacity-80 transition-opacity cursor-pointer"
						aria-label="Linkroot home"
					>
						<LinkrootLogo />
					</a>
					<div className="relative z-10 flex items-center gap-1">
						{socialLinks.map(({ href, label, Icon }) => (
							<a
								...
								target="_blank"
								rel="noreferrer"
								className="relative z-10 p-2 rounded-md text-muted-foreground hover:text-white transition-colors cursor-pointer"
							>
								<Icon size={20} />
							</a>
						))}
					...
					<ul className="flex flex-wrap gap-4 font-medium text-muted-foreground text-sm md:gap-6">
						{navLinks.map((link) => (
							<li key={link.label} className="relative z-10">
								<a 
									className="relative z-10 hover:text-white transition-colors hover:underline decoration-white underline-offset-4 cursor-pointer" 
									href={link.href}
								>
									{link.label}
								</a>
			...
			<div className="relative z-10 flex items-center justify-between gap-4 border-t border-border/10 py-4 px-4 md:px-6 text-muted-foreground text-sm">
				<p>&copy; {new Date().getFullYear()} Linkroot</p>

				<p className="inline-flex items-center gap-1">
					<span>Built by</span>
					<a
						aria-label="Israel's twitter"
						className="relative z-10 inline-flex items-center gap-1.5 text-foreground/60 hover:text-white transition-colors group cursor-pointer"
						href="https://x.com/IFirew91900"
						rel="noreferrer"
						target="_blank"
					>
						<img
							...
							height="auto"
							src="https://unavatar.io/twitter/IFirew91900"
							width="auto"
						/>
						<span className="group-hover:underline decoration-white underline-offset-4 font-medium">Israel</span>
					</a>
				</p>
			</div>
		</footer>
